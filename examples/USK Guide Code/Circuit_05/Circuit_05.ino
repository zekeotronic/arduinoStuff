...
const char notes[] = "b f ddcd ce e";
...
{
  int button1State;
  // button2State;  // variables to hold the pushbutton states
  ...
  if (button1State == 1 && armed == false) {
    digitalWrite(greenPin, HIGH);
  }

  if (button1State == 0 && armed == false) {
    armed = true;
  }
  if (armed) {
    count++;
  ...
  if (button1State == LOW) // pushing button 1 AND button 2
                                                        // then...
  {
